...
const GET_LOCATION_TOOL: Tool = {
    name: "get_location",
    description: "Get user location information, this tool can obtain the latitude and longitude of the user's location",
    inputSchema: {
        type: "object",
        properties: {},
        required: []
...
async function pollLocation(uuid: string): Promise<LocationResponse> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 404) {
                    ...
                    console.error(`Request failed (${attempt}/${maxAttempts}): ${response.status} ${response.statusText}`);
                }
        ...
        if (attempt < maxAttempts) {
            ...
            await new Promise(resolve => setTimeout(resolve, interval + randomDelay));
        }
    }

    // Still no location information after 60 seconds
    console.error(`Polling timeout, failed to get location information within ${maxAttempts} seconds`);
    ...
    try {
        ...
        try {
            ...
            throw new McpError(
                ...
                `Failed to open browser: ${error instanceof Error ? error.message : String(error)}, please visit manually: ${browserUrl}`
            );
        }
