...
async function convertFormat(
  ...
  for (const imagePath of images) {
    try {
      const outputFilePath = generateOutputPath(
        imagePath,
        outputPath,
        "",
        targetFormat,
...
async function compressToSize(
  ...
  for (const imagePath of images) {
    try {
      const outputFilePath = overwrite ? imagePath : generateOutputPath(
        imagePath,
        outputPath,
        `_${targetSize.toLowerCase()}`,
      );
      ...
      if (bestBuffer) {
        ...
        throw new Error(
          "Could not achieve target size while maintaining acceptable quality",
        );
      }
    } catch (error) {
...
async function addTextWatermark(
  ...
  const createTextSvg = (
    ...
    const baseFontSize = customFontSize ||
      Math.max(Math.min(width, height) / 30, 8);
    ...
    if (position === "tile") {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          svgContent +=
            `<text x="${x}" y="${y}" font-family="sans-serif" font-size="${baseFontSize}px" fill="rgba(255,255,255,${opacity})" transform="rotate(${rotateAngle} ${x} ${y})">${text}</text>`;
        }
      }
...
async function handleCompressToSize(
  ...
  try {
    const results = await compressToSize(
      inputPaths,
      size,
      overwrite,
      outputPath,
        ...
        text: JSON.stringify(
          {
            success: true,
            message:
              `成功压缩了 ${results.length} 张图片到目标大小 ${size}，保存在${outputPath}`,
...
async function handleCompressToPercent(
  ...
  try {
    const results = await compressToPercent(
      inputPaths,
      percent,
      overwrite,
      outputPath,
    ...
    return {
      content: [{
        ...
        text: JSON.stringify(
          {
            success: true,
            message: `成功压缩了 ${results.length} 张图片到原始大小的 ${
        ...
        text: JSON.stringify(
          {
            success: true,
            message:
              `成功调整了 ${results.length} 张图片的尺寸，保存在${outputPath}`,
        ...
        text: JSON.stringify(
          {
            success: true,
            message:
              `成功旋转了 ${results.length} 张图片（角度: ${angle}°），保存在${outputPath}`,
        ...
        text: JSON.stringify(
          {
            success: true,
            message: `成功${
              flipType === "horizontal" ? "水平" : "垂直"
            }翻转了 ${results.length} 张图片，保存在${outputPath}`,
...
async function handleWatermark(
  ...
  try {
    ...
    if (
      position === "tile" &&
      (density < 1 || density > 10 || !Number.isInteger(density))
    ) {
    ...
    const results = await addTextWatermark(
      inputPaths,
      text,
      position,
      density,
        ...
        text: JSON.stringify(
          {
            success: true,
            message:
              `成功为 ${results.length} 张图片添加${positionDesc}水印${fontSizeInfo}，保存在${outputPath}`,
  ...
  try {
    switch (request.params.name) {
      case "image_convert_format": {
        const { inputPaths, targetFormat, outputPath } = request.params
          .arguments as {
            inputPaths: string[];
            targetFormat: string;
            outputPath?: string;
      ...
      case "image_compress_to_size": {
        ...
        return await handleCompressToSize(
          inputPaths,
          size,
          overwrite,
          outputPath,
      ...
      case "image_compress_to_percent": {
        ...
        return await handleCompressToPercent(
          inputPaths,
          percent,
          overwrite,
          outputPath,
      ...
      case "image_resize": {
        const { inputPaths, width, height, overwrite, outputPath } = request
          .params.arguments as {
            inputPaths: string[];
            width?: number;
            height?: number;
      ...
      case "image_rotate": {
        const { inputPaths, angle, overwrite, outputPath } = request.params
          .arguments as {
            inputPaths: string[];
            angle: number;
            overwrite?: boolean;
      ...
      case "image_flip": {
        const { inputPaths, flipType, overwrite, outputPath } = request.params
          .arguments as {
            inputPaths: string[];
            flipType: "horizontal" | "vertical";
            overwrite?: boolean;
