...
const readShortcutLink = (filePath: string): Promise<ShortcutInfo> => {
  return new Promise(async (resolve) => {
    ...
    function loadByPowerShell() {
      // 回退 PowerShell（兼容性略差）
      const psCmd = `$sc=(New-Object -COM WScript.Shell).CreateShortcut(\"${filePath.replace(/`/g, '``')}\");Write-Output $sc.TargetPath;`;
      exec(`powershell -NoProfile -NonInteractive -Command "${psCmd}"`, { windowsHide: true }, (error, stdout) => {
    ...
    fs.readdir(filePath, async function (err, files) {
      ...
      for (const filename of files) {
        ...
        try {
          ...
          if (isFile && (path.extname(filename) === '.lnk' || path.extname(filename) === '.url')) {
            ...
            if (path.extname(filename) === '.lnk') {
              ...
              // 尝试 IconFile 字段
              try {
                const iconMatch = fs.readFileSync(filedir, 'utf8').match(/^IconFile=(.*)$/m);
            ...
            try {
              ...
              const source = iconSourcePath && fs.existsSync(iconSourcePath) ? iconSourcePath : targetPath;
