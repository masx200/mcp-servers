  ...
  properties: {
    content: {
      ...
      items: {
        type: "object",
        description:
          "单条热门数据的内容结构 (JSON object with pre-formatted fields)",
        properties: {
          itemDisplayTitle: {
            type: "string",
            description: "列表项的展示标题 (例如: 1. 标题内容)",
          },
          itemTitle: { type: "string", description: "原始标题" },
          itemUrl: {
            type: "string",
            description:
              "格式化的原文链接 (例如: 🔗 链接：https://xxx.com/xxx)",
          },
          itemMobileUrl: {
            type: "string",
            description:
              "格式化的移动端链接 (例如: 📱 移动端链接：https://m.xxx.com/xxx)",
          },
          itemHotness: {
            type: "string",
            description: "格式化的热度 (例如: 🔥 热度：10.5万)",
          },
          itemTimestamp: {
            type: "string",
            description: "格式化的发布时间 (例如: ⏰ 时间：5分钟前)",
          },
          itemDescription: {
            type: "string",
            description: "格式化的内容描述 (例如: 📝 描述：这是描述内容)",
          },
          itemAuthor: {
            type: "string",
            description: "格式化的作者信息 (例如: 👤 作者：张三)",
          },
          itemId: {
            type: "string",
            description: "格式化的唯一标识符 (例如: 🆔 ID：12345)",
          },
        },
    ...
    sites: {
      ...
      items: {
        ...
        properties: {
          siteName: {
            type: "string",
            description: "站点名称 (例如: 哔哩哔哩)",
          },
          siteCategory: {
            type: "string",
            description: "站点类别 (例如: 热门榜)",
          },
          siteIdentifier: {
            type: "string",
            description: "站点的调用名称 (例如: bilibili)",
          },
        },
  ...
  inputSchema: {
    ...
    properties: {
      site: {
        type: "string",
        description:
          `站点名称或调用名称，例如'微博'是'weibo'。不传参数则返回支持的站点列表。\n\n支持的站点列表：\n${generateSiteListDescription()}`,
      },
    },
  },
...
async function handleGetSiteData(siteParam: string) {
  try {
    ...
    if (apiResponse.data.length === 0) {
      throw new McpError(
        ErrorCode.InternalError,
        `API for ${targetSite.站点} returned no data items. This site might have no hot content currently.`,
      );
    }

    // 构建符合schema的JSON数据
    const result = {
      content: apiResponse.data.slice(0, 20).map(
        (item: HotItem, index: number) => {
          const outputItem: {
          ...
          if (item.mobileUrl) {
            outputItem.itemMobileUrl = `📱 移动端链接：${item.mobileUrl}`;
          }
          if (item.hot !== undefined) {
            outputItem.itemHotness = `🔥 热度：${formatHotValue(item.hot)}`;
          }
          if (item.timestamp !== undefined) {
            outputItem.itemTimestamp = `⏰ 时间：${
