...
const notionController: AppController = {
  ...
  getWindowScript: (action: string, bounds: WindowBounds) => {
    ...
    return `tell application "Notion" to set bounds of front window to {${x}, ${y}, ${
      x + width
    }, ${y + height}}`;
  },
  // 可选：提供键盘快捷键备用方案
...
const discordController: AppController = {
  ...
  getWindowScript: (action: string, bounds: WindowBounds) => {
    ...
    const { x, y, width, height } = bounds;
    return `tell application "Discord" to set bounds of front window to {${x}, ${y}, ${
      x + width
    }, ${y + height}}`;
  },
  // 为不支持直接控制的应用提供键盘快捷键
...
const figmaController: AppController = {
  ...
  getWindowScript: (action: string, bounds: WindowBounds) => {
    ...
    // 这里假设它有独立的桌面应用
    return `tell application "Figma" to set bounds of front window to {${x}, ${y}, ${
      x + width
    }, ${y + height}}`;
  },
};
...
// 示例：如何在运行时添加新的控制器
export function addCustomControllers(
  universalController: UniversalAppController,
) {
  // 添加所有自定义控制器
  ...
  return {
    ...
    getWindowScript: (action: string, bounds: WindowBounds) => {
      ...
      if (supportsDirectControl) {
        ...
        return `tell application "${appName}" to set bounds of front window to {${x}, ${y}, ${
          x + width
        }, ${y + height}}`;
      }
    },
